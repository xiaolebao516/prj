...
module Serdes_1x14_DDR(
    ...
    wire data_in_single; // 单端信号线

    // 必须添加 IBUFDS，否则无法连接 FPGA 差分引脚
    IBUFDS #(
        .DIFF_TERM("TRUE"),     // 开启内部 100 欧匹配电阻
        .IOSTANDARD("LVDS")     // 电平标准
    ) ibufds_inst (
        .I(D_p),                // 连到外部 P 引脚
        .IB(D_n),               // 连到外部 N 引脚
        .O(data_in_single)      // 输出单端信号
    );
    ...
    ISERDESE2_inst_master (
        ...
        .D (data_in_single), // 1-bit input: Data input
