...
$buildDir = Join-Path $projectRoot 'build\self-trial-observe-g'
$runtimeDir = Join-Path $buildDir 'debug'
$exePath = Join-Path $runtimeDir 'BoneDensity_SelfTrial.exe'
...
try {
    & $qmake -o Makefile (Join-Path $projectRoot 'BoneDensity.pro') -spec win32-g++ 'CONFIG-=debug_and_release' 'CONFIG-=release' 'CONFIG+=debug' 'CONFIG+=observe_before_g_trial' 'DESTDIR=debug' "QMAKE_CXX=$mingwRoot/bin/g++.exe" "QMAKE_LINK=$mingwRoot/bin/g++.exe" "QMAKE_LINK_C=$mingwRoot/bin/gcc.exe" "QMAKE_CC=$mingwRoot/bin/gcc.exe"
    ...
    Copy-Item -LiteralPath (Join-Path $projectRoot 'docs\guides\self-trial-observe-g.txt') -Destination (Join-Path $runtimeDir 'README.txt') -Force
} finally {
    Pop-Location
}
