...
try {
    & $qmake -o Makefile "..\..\..\tests\calibration_tests.pro" -spec win32-g++ "CONFIG+=debug"
    ...
    & $testExe -txt
    if ($LASTEXITCODE -ne 0) { throw "calibration tests failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}

Write-Host "Calibration tests passed: $testExe"
