...
$buildDir = Join-Path $projectRoot "build\tests\mainwindow-safety"
$testExe = Join-Path $buildDir "debug\mainwindow_safety_tests.exe"
...
try {
    & $qmake -o Makefile "..\..\..\tests\mainwindow_safety_tests.pro" -spec win32-g++ "CONFIG+=debug"
    ...
    & $testExe -txt
    if ($LASTEXITCODE -ne 0) { throw "mainwindow safety tests failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}

Write-Host "MainWindow safety tests passed: $testExe"
