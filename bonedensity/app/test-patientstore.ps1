...
$buildDir = Join-Path $projectRoot "build\tests\patientstore"
$testExe = Join-Path $buildDir "debug\patientstore_tests.exe"
...
try {
    & $qmake -o Makefile "..\..\..\tests\patientstore_tests.pro" -spec win32-g++ "CONFIG+=debug"
    ...
    & $testExe -txt
    if ($LASTEXITCODE -ne 0) { throw "patient store tests failed with exit code $LASTEXITCODE" }
} finally {
    Pop-Location
}

Write-Host "Patient store tests passed: $testExe"
